...
/// <summary>
/// GitHub App OAuth 授权（与网页端 PicX 使用相同的 client_id 与回调地址）
/// </summary>
public sealed class GitHubOAuthService
{
    ...
    /// <summary>
    /// 必须与 PicX GitHub App 注册的回调地址一致（网页端为 http://localhost:4000）
    /// </summary>
    ...
    public async Task<GitHubOAuthResult> AuthorizeWithBrowserAsync(CancellationToken cancellationToken = default)
    {
        ...
        catch (HttpListenerException ex)
        {
            return GitHubOAuthResult.Fail(
                $"无法在 {RedirectUri} 启动本地回调（端口 {OAuthCallbackPort} 可能被占用）。请关闭占用该端口的程序（如 PicX 网页 dev 服务）后重试。详情：{ex.Message}");
        }
    ...
    private static async Task WriteCallbackResponseAsync(HttpListenerContext context, CancellationToken cancellationToken)
    {
        const string html = """
            <!DOCTYPE html>
            <html lang="zh-CN">
            <head><meta charset="utf-8"/><title>PicX-WPF</title></head>
            <body style="font-family:Segoe UI,sans-serif;text-align:center;padding:48px;">
              <h2>授权完成</h2>
              <p>可以关闭此页面并返回 PicX-WPF 桌面客户端。</p>
            </body>
            </html>
            """;
