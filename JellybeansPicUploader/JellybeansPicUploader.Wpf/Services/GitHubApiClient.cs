...
public sealed class GitHubApiClient : IDisposable
{
    ...
    public GitHubApiClient()
    {
        ...
        _httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("JellybeansPicUploader", "1.0"));
    ...
    public async Task<UploadContentResult> InitializeRepositoryReadmeAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var readmeBytes = Encoding.UTF8.GetBytes(JellybeansPicUploaderGitHubConstants.InitializeRepositoryReadmeContent);
        return await UploadContentAsync(
            owner,
            repo,
            branch,
            "README.md",
            JellybeansPicUploaderGitHubConstants.InitializeRepositoryCommitMessage,
            readmeBytes,
            owner,
            null,
            cancellationToken).ConfigureAwait(false);
        ...
        Action<GitHubContentUploadPhase>? reportUploadPhase = null)
    {
        ...
        var payload = new Dictionary<string, object?>
        {
            ["message"] = string.IsNullOrWhiteSpace(commitMessage) ? "Upload by JellybeansPicUploader" : commitMessage,
        ...
        CancellationToken cancellationToken = default)
    {
        var payload = new
        {
            message = string.IsNullOrWhiteSpace(commitMessage) ? "Delete by JellybeansPicUploader" : commitMessage,
            sha,
            branch
        };
        ...
        CancellationToken cancellationToken)
    {
        var payload = new
        {
            message = string.IsNullOrWhiteSpace(commitMessage) ? "Batch operation by JellybeansPicUploader" : commitMessage,
            tree = treeSha,
