...
namespace JellybeansPicUploader.ViewModels;
...
public sealed class ShellViewModel : ObservableObject, IDisposable
{
    ...
    private Bitmap? _sidebarAvatarImage;
    ...
    public ShellViewModel()
    {
        ...
        AddImagesCommand = new AsyncRelayCommand(AddImagesFromDialogAsync, () => !IsBusy);
    ...
    public bool IsDarkThemeEnabled
    {
        ...
        set
        {
            ...
            ApplyApplicationThemeVariant(value);
        }
    }

    public Bitmap? SidebarAvatarImage
    {
        get => _sidebarAvatarImage;
        private set
        {
            if (SetProperty(ref _sidebarAvatarImage, value))
            {
                OnPropertyChanged(nameof(HasSidebarAvatarImage));
            }
        }
    }

    public bool HasSidebarAvatarImage => SidebarAvatarImage is not null;
    ...
    {
        var imagePaths = UploadInputHelper.CollectImagePathsFromClipboardAsync().GetAwaiter().GetResult();
    ...
    private async Task<bool> EnsureRepositoryExistsAsync(CancellationToken cancellationToken)
    {
        ...
        var createResult = await _gitHubApiClient.CreateRepositoryAsync(
            ...
            Repository,
            CurrentUser?.Login,
            JellybeansPicUploaderGitHubConstants.DefaultRepositoryDescription,
            isPrivate: false,
    ...
    {
        var imageUrl = await DesktopDialogService.PromptTextAsync("请输入图片 URL（http / https）：").ConfigureAwait(true);
    ...
    private async Task AddImagesFromDialogAsync()
    {
        try
        {
            var selectedFiles = await DesktopDialogService.PickImageFilesAsync(allowMultiple: true).ConfigureAwait(true);
            ...
            if (selectedFiles.Count > 0)
            {
                ...
                StatusText = $"已添加 {selectedFiles.Count} 张图片";
            }
        }
        catch (Exception exception)
        {
            StatusText = $"选择图片失败：{exception.Message}";
        }
    }
    ...
    private void RunOnUiThread(Action action) => UiThreadHelper.RunOnUiThread(action);
    ...
    private async Task DeleteSelectedImagesAsync()
    {
        ...
        try
        {
            ...
            if (selectedImages.Count == 1)
            {
                ...
                var result = await _gitHubApiClient.DeleteContentAsync(Owner, Repository, branch, image.Path, image.Sha, "Delete image by JellybeansPicUploader", cancellationToken).ConfigureAwait(true);
                if (!result.Success)
                {
            ...
            else
            {
                var batchResult = await _gitHubApiClient.DeleteMultipleAsync(
                    ...
                    Repository,
                    branch,
                    "Delete images by JellybeansPicUploader",
    ...
    {
        _ = SelectToolboxFileAsync();
    }

    private async Task SelectToolboxFileAsync()
    {
        IReadOnlyList<string>? selectedFiles;
        try
        {
            selectedFiles = await DesktopDialogService.PickImageFilesAsync(allowMultiple: false).ConfigureAwait(true);
        }
        catch (Exception exception)
        {
            StatusText = $"选择图片失败：{exception.Message}";
        ...
        {
            ToolboxSourceFilePath = selectedFiles[0];
        }
    }
    ...
    private string BuildToolboxOutputPath(string suffix)
    {
        ...
        var outputDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "JellybeansPicUploader", "Toolbox");
    ...
    public void InitializeApplicationThemeFromCurrent()
    {
        ...
        var requestedVariant = application.RequestedThemeVariant;
        var isDarkTheme = requestedVariant == ThemeVariant.Dark
            || (requestedVariant == ThemeVariant.Default
                && application.ActualThemeVariant == ThemeVariant.Dark);

        _isDarkThemeEnabled = isDarkTheme;
        OnPropertyChanged(nameof(IsDarkThemeEnabled));
    }

    private static void ApplyApplicationThemeVariant(bool isDarkThemeEnabled)
    {
        ...
        application.RequestedThemeVariant = isDarkThemeEnabled ? ThemeVariant.Dark : ThemeVariant.Light;
    }

    private async Task RefreshSidebarAvatarImageAsync()
    {
        ...
        try
        {
            var avatarBitmap = await _remoteThumbnailLoader
                .TryLoadAsync(avatarUrl, GitHubToken, CancellationToken.None)
                .ConfigureAwait(true);
            SidebarAvatarImage = avatarBitmap;
        }
        catch
        {
            SidebarAvatarImage = null;
    ...
    private void RaiseLoginStateChanged()
    {
        OnPropertyChanged(nameof(IsLoggedIn));
        OnPropertyChanged(nameof(LoginDisplayName));
        _ = RefreshSidebarAvatarImageAsync();
    ...
    private sealed class UploadProgressSmoother : IDisposable
    {
        ...
        public UploadProgressSmoother(int fromPercent, int toPercent, Action<int> onPercentChanged)
        {
            ...
            _timer = new Avalonia.Threading.DispatcherTimer { Interval = TimeSpan.FromMilliseconds(180) };
